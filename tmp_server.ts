...
app.post("/api/get-journal-notes", async (req, res) => {
  try {
    ...
    const encodedRange = encodeURIComponent("心得筆記!A1");
    const getUrl = `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${encodedRange}`;
...
app.post("/api/save-journal-notes", async (req, res) => {
  try {
    const { accessToken, spreadsheetId, notes } = req.body;
    if (!accessToken || !spreadsheetId) {
      return res.status(400).json({ error: "Missing required fields" });
    }

    // 1. Ensure sheet exists
    const getMetaUrl = `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}`;
    ...
    if (metaRes.ok) {
      ...
      if (!hasNotesSheet) {
        ...
        await fetch(batchUrl, {
          ...
          body: JSON.stringify({
            requests: [{ addSheet: { properties: { title: '心得筆記' } } }]
          }),
        });
      }
    }
